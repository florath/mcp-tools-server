...
from src.mcp_tools_server.tools.file_mover import FileMoverTool
...
@pytest.fixture
def file_mover(security_validator):
...
@pytest.mark.asyncio
async def test_file_mover_basic_move(file_mover, test_file, security_validator):
    ...
    result = await file_mover.execute(params)
...
@pytest.mark.asyncio
async def test_file_mover_to_subdirectory(file_mover, test_file, security_validator):
...
@pytest.mark.asyncio
async def test_file_mover_overwrite_with_backup(file_mover, test_file, existing_destination, security_validator):
...
@pytest.mark.asyncio
async def test_file_mover_overwrite_without_backup(file_mover, test_file, existing_destination, security_validator):
...
@pytest.mark.asyncio
async def test_file_mover_no_overwrite_fails(file_mover, test_file, existing_destination, security_validator):
    ...
    with pytest.raises(ValueError, match="Destination file already exists"):
        await file_mover.execute(params)
...
@pytest.mark.asyncio
async def test_file_mover_nonexistent_source(file_mover, security_validator):
...
@pytest.mark.asyncio
async def test_file_mover_directory_as_source(file_mover, security_validator):
...
@pytest.mark.asyncio
async def test_file_mover_missing_parameters(file_mover):
...
def test_file_mover_tool_info():
    """Test file mover tool info includes reason parameter."""
    tool = FileMoverTool()
    info = tool.get_info()
    
    assert info["name"] == "file_mover"
    assert "reason" in info["parameters"]["required"]
    assert info["parameters"]["properties"]["reason"]["minLength"] == 10


@pytest.mark.asyncio
async def test_file_mover_rename_in_same_directory(file_mover, test_file, security_validator):
